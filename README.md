...
The monitor font is proportional, meaning each character does not take the same pixel space when it is displayed, so some characters will take more spaces than others. For example, the `\` take twice as more space than the `.`, so the limit will be around 8 to 12 depending on the characters you are using.
